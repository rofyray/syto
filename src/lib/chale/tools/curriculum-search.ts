 ...
 * Allows Chale to search the Weaviate curriculum database
 */
export const curriculumSearchTool: Anthropic.Tool = {
  name: 'search_curriculum',
  description: `Search Ghana's national curriculum database for relevant educational content. Use this BEFORE generating any educational content to ensure curriculum alignment. The database contains authentic Ghana Ministry of Education syllabus content for Primary 4-6.`,
  input_schema: {
    type: 'object',
    properties: {
      subject: {
...
export async function handleCurriculumSearch(input: unknown): Promise<string> {
  try {
    ...
    // Query Weaviate for curriculum content
