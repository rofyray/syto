...
app.get('/health', cacheMiddleware(5 * 60 * 1000), (_req, res) => {
    res.json({
        status: 'healthy',
...
app.use('/api/chale', optionalAuth, // Optional authentication
logUserActivity, // Log user activity
smartCache, // Smart caching based on content type
chaleRoutes // Main Chale routes
);
// Protected routes that require authentication
app.use('/api/chale/protected', authenticateUser, // Require authentication
logUserActivity, // Log user activity
chaleRoutes // Main Chale routes
);
...
export const handler = async (event, context) => {
    ...
    if (typeof result === 'object' && result !== null && 'statusCode' in result) {
        return result; // Safe to cast as we've verified it has statusCode
    }
