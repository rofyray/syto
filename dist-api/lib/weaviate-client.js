 ...
 */
export async function queryMathDB(query, grade, limit = 10) {
 ...
 */
export async function queryReadingDB(query, grade, limit = 10) {
    const client = await getWeaviateClient();
    try {
 ...
 */
export async function searchCurriculumContent(query, subject, grade, limit = 5) {
    try {
 ...
 */
export async function getCurriculumContext(subject, grade, topic) {
    try {
        ...
        const contextParts = results.map((item, index) => `Context ${index + 1}: ${item.properties?.content || item.content || 'No content available'}`);
