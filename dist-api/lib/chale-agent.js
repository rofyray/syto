...
import { createChaleAgent, generateChaleSystemPrompt, validateChaleContent } from './chale-config.js';
...
export class ChaleAgent {
    ...
    async generateContent(request) {
        ...
        try {
            ...
            const curriculumContext = await getCurriculumContext(request.subject, request.grade, request.topic || `${request.subject} ${request.type}`);
     ...
     */
    buildGenerationPrompt(request, curriculumContext) {
...
Generate the content now as a JSON object:`;
     ...
     */
    async validateAgainstCurriculum(content) {
        try {
            // Get relevant curriculum context
            const curriculumContext = await getCurriculumContext(content.subject, content.grade, content.title);
